...
desc: (legacy @quasar/app-vite v1) How to convert a Quasar CLI with Webpack project to a Quasar CLI with Vite one.
---
...
There are property changes in `build`, `devServer`, and all Quasar Modes (pwa, ssr, etc). The props are detailed in the [quasar.config file](/quasar-cli-vite-v1/quasar-config-file) page. You will have to manually port your configuration to the Quasar CLI with Vite architecture.
...
A Quasar CLI with Webpack project relies on `/package.json > browserslist` to specify which browsers you are targeting. That property no longer has any meaning. Projects managed by Quasar CLI with Vite work completely different and you might want to check the [Browser Compatibility](/quasar-cli-vite-v1/browser-compatibility) page.

### Step 6: SSR related

* Delete `/src-ssr/directives` folder (if you have it) -- it no longer serves any purpose; check [Vue SSR Directives](/quasar-cli-vite-v1/developing-ssr/vue-ssr-directives) page
* Port the `/src-ssr/production-export.js` file to `/src-ssr/server.js`; Make sure to read about the [SSR Webserver](/quasar-cli-vite-v1/developing-ssr/ssr-webserver) first

More info: [Configuring SSR](/quasar-cli-vite-v1/developing-ssr/configuring-ssr)
...
* The default name of the outputted service worker file has changed from `service-worker.js` to `sw.js`. This can break your update process the first time the new app is loaded. So, if your app is in production, to ensure smooth upgrades from the previous Webpack builds, make sure the name matches the name of your previous service worker file. You can set it through [quasar.config file > pwa > swFilename](/quasar-cli-vite-v1/developing-pwa/configuring-pwa#quasar-config-file).
...
More info: [PWA - Preparation](/quasar-cli-vite-v1/developing-pwa/preparation)
...
More info: [Preparation and folder structure](/quasar-cli-vite-v1/developing-browser-extensions/preparation#2-understand-the-anatomy-of-src-bex) and [Configuring BEX](/quasar-cli-vite-v1/developing-browser-extensions/configuring-bex).
